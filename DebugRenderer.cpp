...
Adina::DebugRenderer::DebugRenderer() {
	// Empty
}
...
void Adina::DebugRenderer::drawCircle(const glm::vec2& center, const ColorRGBA8& color, float radius) {
...
void Adina::DebugRenderer::render(const glm::mat4& projectionMatrix, float lineWidth) {
	m_program.use();

	m_program.setUniformMatrix4fv("P", projectionMatrix);
...
void Adina::DebugRenderer::dispose() {
	if (m_vao) {
